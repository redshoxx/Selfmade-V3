...
async function body(req){if(req.method==='GET'||req.method==='HEAD')return undefined;let total=0,chunks=[];for await(const chunk of req){total+=chunk.length;if(total>5*1024*1024)throw new Error('request_too_large');chunks.push(chunk)}if(!chunks.length)return{};const text=Buffer.concat(chunks).toString('utf8');if((req.headers['content-type']||'').includes('application/json')){try{return JSON.parse(text)}catch{return text}}return text}
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url||'/',`http://${req.headers.host||`${HOST}:${PORT}`}`);if(url.pathname==='/api/product-lookup')return runApi(productHandler,req,res,url);if(url.pathname==='/api/import-transaction')return runApi(importHandler,req,res,url);if(url.pathname==='/api/manual-sync')return runApi(manualSyncHandler,req,res,url);return serveStatic(req,res,url)})
server.listen(PORT,HOST,()=>{console.log('');console.log('========================================');console.log(' NEST 4.4 lokaler Server läuft');console.log(` http://localhost:${PORT}`);console.log(' Local-First & Manual Sync Build r21');console.log(' Zum Beenden STRG+C drücken');console.log('========================================');console.log('')})
