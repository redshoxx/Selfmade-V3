...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url||'/',`http://${req.headers.host||`${HOST}:${PORT}`}`);if(url.pathname==='/api/product-lookup')return runApi(productHandler,req,res,url);if(url.pathname==='/api/import-transaction')return runApi(importHandler,req,res,url);return serveStatic(req,res,url)})
server.listen(PORT,HOST,()=>{console.log('');console.log('========================================');console.log(' NEST 4.4 lokaler Server läuft');console.log(` http://localhost:${PORT}`);console.log(' Performance & Stability Build r19');console.log(' Zum Beenden STRG+C drücken');console.log('========================================');console.log('')})
