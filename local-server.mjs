...
const lidlHandler=require('./api/lidl-offers.js')
...
async function serveStatic(req,res,url){let pathname=url.pathname;if(pathname==='/')pathname='/index.html';let file=safeFile(pathname);if(!file){res.statusCode=400;return res.end('Bad request')}
  if(!(await exists(file))&&!path.extname(file)){const html=file+'.html';if(await exists(html))file=html}
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url||'/',`http://${req.headers.host||`${HOST}:${PORT}`}`);if(url.pathname==='/api/product-lookup')return runApi(productHandler,req,res,url);if(url.pathname==='/api/import-transaction')return runApi(importHandler,req,res,url);if(url.pathname==='/api/lidl-offers')return runApi(lidlHandler,req,res,url);return serveStatic(req,res,url)})
server.listen(PORT,HOST,()=>{console.log('');console.log('========================================');console.log(' NEST 4.1 lokaler Server läuft');console.log(` http://localhost:${PORT}`);console.log(' Lidl-Angebote werden automatisch geladen');console.log(' Zum Beenden STRG+C drücken');console.log('========================================');console.log('')})
