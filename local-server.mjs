...
const manualSyncHandler=require('./api/manual-sync.js')
const screenshotHandler=require('./api/screenshot-transaction.js')
...
function apiResponse(res){return{status(code){res.statusCode=code;return this},setHeader(name,value){res.setHeader(name,value);return this},json(body){if(!res.headersSent)res.setHeader('Content-Type','application/json; charset=utf-8');res.end(JSON.stringify(body));return body},end(body){res.end(body);return body},get headersSent(){return res.headersSent},get writableEnded(){return res.writableEnded},set statusCode(value){res.statusCode=value},get statusCode(){return res.statusCode}}}
async function body(req){if(req.method==='GET'||req.method==='HEAD')return undefined;let total=0,chunks=[];for await(const chunk of req){total+=chunk.length;if(total>10*1024*1024)throw new Error('request_too_large');chunks.push(chunk)}if(!chunks.length)return{};const text=Buffer.concat(chunks).toString('utf8');if((req.headers['content-type']||'').includes('application/json')){try{return JSON.parse(text)}catch{return text}}return text}
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url||'/',`http://${req.headers.host||`${HOST}:${PORT}`}`);if(url.pathname==='/api/product-lookup')return runApi(productHandler,req,res,url);if(url.pathname==='/api/import-transaction')return runApi(importHandler,req,res,url);if(url.pathname==='/api/manual-sync')return runApi(manualSyncHandler,req,res,url);if(url.pathname==='/api/screenshot-transaction')return runApi(screenshotHandler,req,res,url);return serveStatic(req,res,url)})
server.listen(PORT,HOST,()=>{console.log('');console.log('========================================');console.log(' NEST 4.4 lokaler Server läuft');console.log(` http://localhost:${PORT}`);console.log(' Local-First, Screenshot Import & Manual Sync r22');console.log(' Zum Beenden STRG+C drücken');console.log('========================================');console.log('')})
