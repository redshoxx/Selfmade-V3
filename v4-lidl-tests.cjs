...
const css=fs.readFileSync('v4-lidl.css','utf8'),index=fs.readFileSync('index.html','utf8'),settings=fs.readFileSync('settings-v3.js','utf8'),sw=fs.readFileSync('sw.js','utf8'),manifest=fs.readFileSync('manifest.webmanifest','utf8')
ok(css.includes('.v4-lidl-entry')&&css.includes('.v4-shop-offer')&&css.includes('.v4-lidl-dialog'),'V4 UI Styles fehlen')
ok(index.includes('name="nest-version" content="4.0.0"')&&index.includes('/v4-lidl.js?v=4.0.0-r14'),'V4 wird nicht direkt aus index.html geladen')
ok(settings.includes('lidl:lidl()?.exportData?.()')&&settings.includes('lidl()?.reset?.()'),'Lidl-Daten fehlen im vollständigen Backup/Reset')
ok(sw.includes("nest-v4.0.0-r14")&&sw.includes('/v4-lidl.js?v=4.0.0-r14'),'V4 Service-Worker-Cache fehlt')
ok(manifest.includes('NEST 4')&&manifest.includes('Lidl-Angebotsintelligenz'),'V4 PWA-Metadaten fehlen')
