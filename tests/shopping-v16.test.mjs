...
test('shopping page uses the V16 quick-add workflow', () => {
  assert.match(app, /Was brauchst du\?/);
  assert.match(app, /shopping-quick-v16/);
  assert.match(app, /open-shopping-details-v16/);
  assert.match(app, /shoppingCatalogMatchesV16/);
  assert.match(app, /parseShoppingDraftV16/);
});

test('V16 shopping renderer only displays real data suggestions', () => {
  assert.match(app, /const suggestions = data\.suggestions \|\| \[\]/);
...
test('shopping layout is compact and usable on iPhone 12', () => {
  ...
  assert.match(styles, /min-height:\s*65px/);
