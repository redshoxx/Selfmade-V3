...
function prepareAppSource(app) {
  const prepared = app
    ...
    .replace('nutze den grünen Plus-Button', 'nutze den Plus-Button')
  ...
  if (prepared.includes('grünen Plus-Button')) {
    throw new Error('Veralteter Grün-Hinweis ist noch in der App enthalten.')
  }
  return prepared
}
...
for (const required of [
  'selfmade-version" content="2.1.1',
  'window.__SELFMADE_READY__ = true',
  'data-app-ready',
  'id="app"',
  'id="sheet"',
...
if (html.includes('type="module"') || html.includes('src="/app.js') || html.includes('grünen Plus-Button')) {
  throw new Error('Veraltete oder externe Startlogik ist noch aktiv.')
}
