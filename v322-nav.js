...
function injectStyle(){
  ...
  style.textContent=`
    ...
    @media(max-width:430px){
      .v322-top-actions{gap:6px}
      .v322-home-btn,.v322-top-actions .v3-icon-btn{width:39px!important;height:39px!important;border-radius:12px!important}
      .v3-nav.v31-nav.v322-nav{grid-template-columns:repeat(5,minmax(0,1fr))!important;padding-left:6px!important;padding-right:6px!important}
      .v3-nav.v31-nav.v322-nav>button span{font-size:7px!important}
    }
  `
...
function enhance(){
  ...
  if(nav){
    ...
    const oldHome=nav.querySelector('[data-route="overview"]')
    if(oldHome)oldHome.remove()
  }
  const top=document.querySelector('.v3-top')
  if(!top)return
  const settings=top.querySelector('[data-v3="settings"]')
  let actions=top.querySelector('.v322-top-actions')
  if(!actions){
    actions=document.createElement('div')
    actions.className='v322-top-actions'
    if(settings)top.insertBefore(actions,settings);else top.appendChild(actions)
  }
  let home=actions.querySelector('.v322-home-btn')
  if(!home){
    home=document.createElement('button')
    home.type='button'
    home.className='v322-home-btn'
    home.setAttribute('data-route','overview')
    home.setAttribute('aria-label','Übersicht')
    home.setAttribute('title','Übersicht')
    home.innerHTML=HOME
    actions.appendChild(home)
  }
  if(settings&&settings.parentElement!==actions)actions.appendChild(settings)
  const page=document.querySelector('.v3-main')?.dataset.page
  home.classList.toggle('active',page==='overview')
}
...
root.NestNavV322={RELEASE,enhance,schedule}
})(globalThis)
