...
function readAudit(){var a=parse(storage.getItem(AUDIT_KEY),null);if(a&&Array.isArray(a.events))return{version:2,events:a.events};if(Array.isArray(a))return{version:2,events:a};return{version:2,events:[]}}
...
function writeAudit(a){try{writeVerified(AUDIT_KEY,JSON.stringify(a));return true}catch(e){storageError=String(e&&e.message||e);return false}}
...
function backupEnvelope(state){var clean=normalizeState(state),payload=JSON.stringify(clean);return JSON.stringify({schema:1,release:RELEASE,savedAt:iso(),checksum:hashString(payload),state:clean})}
...
function writeBackup(state){try{writeVerified(BACKUP_KEY,backupEnvelope(state));return true}catch(e){storageError=String(e&&e.message||e);return false}}
function rawState(){var p=parse(storage.getItem(CORE_KEY),null);return validState(p)?normalizeState(p):null}
function loadState(){var s=rawState(),b;if(!s){b=readBackup();s=b||initialState();try{writeVerified(CORE_KEY,JSON.stringify(s))}catch(e){storageError=String(e&&e.message||e)}}writeBackup(s);return clone(s)}
function saveState(next,opt){var prev=rawState()||initialState(),clean=normalizeState(next),method=opt&&opt.method||'manual',a=ensureAudit(prev,readAudit());clean.version=RELEASE;writeVerified(CORE_KEY,JSON.stringify(clean));transition(prev,clean,a,method);writeAudit(a);writeBackup(clean);return clone(clean)}
function loadPrefs(){return normalizePrefs(parse(storage.getItem(PREF_KEY),{}))}
function savePrefs(p){var clean=normalizePrefs(p);writeVerified(PREF_KEY,JSON.stringify(clean));return clone(clean)}
function replaceAll(payload){payload=payload||{};var src=payload.core||payload.state||payload;if(!validState(src))throw new Error('Ungültiger NEST Datenstand');var s=normalizeState(src);writeVerified(CORE_KEY,JSON.stringify(s));if(payload.audit)writeAudit(payload.audit);if(payload.preferences)savePrefs(payload.preferences);writeBackup(s);return clone(s)}
function resetAll(){storage.removeItem(CORE_KEY);storage.removeItem(BACKUP_KEY);storage.removeItem(AUDIT_KEY);storage.removeItem(PREF_KEY)}
...
function status(){return{available:persistent,persistent:persistent,lastError:storageError,coreKey:CORE_KEY,backupKey:BACKUP_KEY,release:RELEASE}}
var initial=loadState();writeAudit(ensureAudit(initial,readAudit()))
