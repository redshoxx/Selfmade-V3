...
assert.ok(index.includes('/native-v44.js?v=4.4.0-r22'),'Native Bridge wird nicht geladen')
assert.ok(index.includes('/screenshot-import-v44.js?v=4.4.0-r22'),'Screenshot-Import fehlt in iOS Assets')
assert.ok(index.indexOf('/native-v44.js')<index.indexOf('/app-v3.js'),'Native Bridge muss vor App-Code laden')
assert.ok(index.indexOf('/screenshot-import-v44.js')<index.indexOf('/app-v3.js'),'Screenshot-Import muss vor App-Code laden')
assert.ok(sw.includes("nest-v4.4.0-r22")&&sw.includes('/native-v44.js?v=4.4.0-r22'),'Native Bridge fehlt im PWA-Cache')
assert.ok(sw.includes('/screenshot-import-v44.js?v=4.4.0-r22'),'Screenshot-Import fehlt im Cache')
...
assert.ok(workflow.includes('NEW_ID="at.nest.selfmade.embedded.f${INDEX}"'),'Eindeutige Embedded-Bundle-ID-Reparatur fehlt')
assert.ok(workflow.includes('Set :CFBundleIdentifier $NEW_ID'),'Embedded Bundle-Identifier werden nicht repariert')
assert.ok(workflow.includes('Bundle-ID repariert:'),'Bundle-ID-Reparatur ist im Build nicht nachvollziehbar')
assert.ok(workflow.includes('Embedded bundle $RELATIVE verwendet fälschlich die App-ID'),'Framework/App Bundle-ID-Kollision wird nach Reparatur nicht geprüft')
...
assert.ok(workflow.includes("- 'screenshot-import-v44.js'")&&workflow.includes("- 'api/screenshot-transaction.js'"),'Screenshot-Änderungen lösen keinen iOS-Build aus')
...
assert.equal(context.NestNativeV44.apiUrl('/api/manual-sync'),'https://selfmade-v3.vercel.app/api/manual-sync')
assert.equal(context.NestNativeV44.apiUrl('/api/screenshot-transaction'),'https://selfmade-v3.vercel.app/api/screenshot-transaction')
assert.equal(context.document.documentElement.dataset.nestNative,'ios')

console.log('NEST V4.4.0 iOS Capacitor, screenshot import, unique embedded bundle IDs and Sideloadly IPA regression tests passed')
